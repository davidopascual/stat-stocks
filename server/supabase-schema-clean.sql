...
DO $$ 
BEGIN
  -- Add positions foreign key if not exists
  IF NOT EXISTS (
  ...
  -- Add transactions foreign key if not exists
  IF NOT EXISTS (
  ...
  -- Add leagues foreign key if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'leagues_creator_id_fkey'
  ) THEN
    ALTER TABLE leagues 
  ...
  -- Add option_positions foreign key if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'option_positions_user_id_fkey'
  ) THEN
  ...
  -- Add leaderboard league foreign key if not exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'leaderboard_entries_league_id_fkey'
  ) THEN
  ...
  -- Add leaderboard user foreign key if not exists
  IF NOT EXISTS (
...
-- Disable RLS for development (enable and add proper policies for production)
...
BEGIN
  ...
  RAISE NOTICE 'Tables created: users, positions, transactions, leagues, option_positions, leaderboard_entries';
  RAISE NOTICE 'Note: RLS is DISABLED for development. Enable for production!';
END $$;
